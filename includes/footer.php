        </div>
    </div>

    <!-- Footer -->
    <footer class="bg-black border-t border-gray-800 py-16 px-4">
        <div class="max-w-7xl mx-auto">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-12">
                <div>
                    <h3 class="text-xl font-serif font-bold text-gold mb-6">Château Lumière</h3>
                    <p class="text-gray-400 mb-6">
                        A culinary journey of exceptional flavors and impeccable service in an atmosphere of refined luxury.
                    </p>
                    <div class="flex space-x-4">
                        <a href="#" class="text-gray-400 hover:text-gold transition duration-300">
                            <i class="fab fa-facebook-f"></i>
                        </a>
                        <a href="#" class="text-gray-400 hover:text-gold transition duration-300">
                            <i class="fab fa-instagram"></i>
                        </a>
                        <a href="#" class="text-gray-400 hover:text-gold transition duration-300">
                            <i class="fab fa-twitter"></i>
                        </a>
                    </div>
                </div>
                
                <div>
                    <h4 class="text-lg font-medium mb-6">Hours</h4>
                    <ul class="space-y-3 text-gray-400">
                        <li>Monday - Thursday: 5:30PM - 10:30PM</li>
                        <li>Friday - Saturday: 5:00PM - 11:30PM</li>
                        <li>Sunday: 5:00PM - 9:30PM</li>
                    </ul>
                </div>
                
                <div>
                    <h4 class="text-lg font-medium mb-6">Location</h4>
                    <address class="text-gray-400 not-italic">
                        123 Luxury Avenue<br>
                        Beverly Hills, CA 90210<br>
                        <br>
                        <a href="tel:+18005551234" class="hover:text-gold transition duration-300">(+62) 895-1293-7862</a><br>
                        <a href="mailto:reservations@elegance.com" class="hover:text-gold transition duration-300">chateaulumiere27@gmail.com</a>
                    </address>
                </div>
                
                <div>
                    <h4 class="text-lg font-medium mb-6">Newsletter</h4>
                    <p class="text-gray-400 mb-4">
                        Subscribe to receive updates on special events and exclusive offers.
                    </p>
                    <form class="flex">
     
                    </form>
                </div>
            </div>
            
            <div class="border-t border-gray-800 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center">
                <p class="text-gray-400 text-sm">
                    &copy; <?php echo date('Y'); ?> Château Lumière. All rights reserved.
                </p>
                <div class="flex space-x-6 mt-4 md:mt-0">
                </div>
            </div>
        </div>
    </footer>

    <!-- JavaScript -->
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script src="/assets/js/main.js"></script>
</body>
</html>