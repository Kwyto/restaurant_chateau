...
<style>
    ...
    * {
    ...
    @keyframes pulse {
        ...
        100% {
            text-shadow: 0 0 15px var(--gold), 0 0 30px rgba(212, 175, 55, 0.8);
        }
    }
    ...
    /* --- START: GAYA UNTUK TESTIMONIAL CAROUSEL (DIPERBARUI) --- */
    ...
    .testimonial-carousel-container.is-dragging {
        cursor: grabbing; /* Kursor saat sedang di-drag */
    }

    /* Menghilangkan seleksi teks saat dragging */
    .testimonial-carousel-container.is-dragging .testimonial-card {
        user-select: none;
        pointer-events: none;
    }
    /* --- END: GAYA UNTUK TESTIMONIAL CAROUSEL --- */
...
<main class="min-h-screen bg-black text-white overflow-x-hidden">
        ...
        <div class="relative z-20 text-center px-4 max-w-4xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-6 opacity-0" style="animation: fadeIn 1.5s ease-out 1.5s forwards;">
                <?php if (isset($_SESSION['user_id'])) :
                    // Kita sudah mengambil $userData di atas, jadi bisa langsung digunakan
                ?>
                    <a href="pages/profile" class="px-10 py-5 btn-luxury text-gold hover:text-black transition-all duration-700 font-medium tracking-wide">
                        <?php echo getGreeting() . ', ' . ($userData['gender'] == 'male' ? 'Mr.' : 'Mrs.') . ' ' . htmlspecialchars($userData['first_name']); ?>
                    </a>
                <?php else : ?>
        ...
        <div class="grid md:grid-cols-2 gap-20 items-center relative z-10">
            <div data-aos="fade-right" data-aos-duration="1200">
                ...
                <p class="text-lg leading-relaxed mb-8 opacity-80">
                    Founded in 1975, Château Lumière has consistently ranked among the world's finest dining establishments.
            ...
            <?php while ($item = mysqli_fetch_assoc($featuredItems)) : ?>
                <div class="card-luxury menu-item-hover" data-aos="zoom-in" data-aos-duration="800">
                    <div class="luxury-image h-72 overflow-hidden">
                        <img src="assets/images/menu/<?php echo $item['image_path']; ?>" alt="<?php echo htmlspecialchars($item['name']); ?>" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                        <div class="menu-item-overlay"></div>
                    </div>
                    <div class="p-8">
                        <h3 class="text-2xl font-serif font-bold mb-3 group-hover:text-gold transition-colors duration-300"><?php echo htmlspecialchars($item['name']); ?></h3>
                        <p class="text-gray-400 mb-6 leading-relaxed"><?php echo htmlspecialchars($item['description']); ?></p>
    ...
    <section class="py-32 px-4 max-w-7xl mx-auto section-luxury rounded-3xl my-16 relative overflow-hidden">
        ...
        <div id="testimonial-carousel-container" class="relative overflow-hidden testimonial-carousel-container">
            <div id="testimonial-track" class="flex gap-8 py-4">
                <?php if (!empty($testimonials)) : ?>
                    <?php foreach ($testimonials as $testimonial) : ?>
                        <div class="testimonial-card flex-shrink-0 w-80 md:w-96 p-8 rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-gold transition-all">
                            <div class="flex items-center mb-6">
                                <div class="w-14 h-14 rounded-full bg-gradient-to-r from-gold to-gold-light flex items-center justify-center text-black font-bold mr-4 text-xl">
                                ...
                                <div>
                                    ...
                                    <div class="flex text-gold mt-1">
                                        <?php for ($i = 0; $i < 5; $i++) : ?>
                                            <svg class="w-5 h-5" fill="<?php echo $i < $testimonial['rating'] ? 'currentColor' : 'none'; ?>" stroke="currentColor" viewBox="0 0 20 20">
                ...
                <form method="POST" action="">
                    <div class="mb-8">
                        ...
                        <div class="flex justify-center space-x-2">
                            <?php for ($i = 1; $i <= 5; $i++) : ?>
                                <input type="radio" id="star<?php echo $i; ?>" name="rating" value="<?php echo $i; ?>" class="hidden" <?php echo $i == 5 ? 'checked' : ''; ?>>
                    ...
                    <div class="mb-8">
                        ...
                        <textarea id="comment" name="comment" rows="5" class="w-full bg-gray-800 border border-gray-700 rounded-lg p-4 text-white focus:border-gold focus:ring-gold" placeholder="Tell us about your experience..." required></textarea>
                    </div>

                    <button type="submit" name="submit_testimonial" class="w-full py-4 bg-gradient-to-r from-gold to-gold-light text-black font-bold rounded-lg hover:from-gold-light hover:to-gold transition-all duration-300">
                        Submit Testimonial
                    </button>
                </form>
            <?php else : ?>
                <div class="text-center py-10">
                    ...
                    <a href="pages/auth/login.php" class="px-8 py-4 bg-gradient-to-r from-gold to-gold-light text-black font-bold rounded-lg hover:from-gold-light hover:to-gold transition-all duration-300 inline-block">
                        Login Now
                    </a>
                </div>
            <?php endif; ?>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // --- START: KODE JAVASCRIPT UNTUK CAROUSEL INTERAKTIF (DIPERBARUI) ---
        ...
        if (container && track) {
            ...
            if (cards.length > 0) {
                // Gandakan kartu untuk loop yang mulus
                cards.forEach(card => {
                ...
                let startX;
                let trackX; // Posisi awal track saat drag dimulai
                let currentX = 0; // Posisi transformX saat ini
                let animationFrameId;

                // Kecepatan scroll otomatis (pixel per frame). Lebih besar = lebih cepat.
                ...
                const animate = () => {
                    if (!isDragging) {
                        ...
                        // Cek jika sudah scroll sejauh set kartu original, reset ke awal
                        const originalContentWidth = track.scrollWidth / 2;
                        if (currentX <= -originalContentWidth) {
                            currentX %= originalContentWidth;
                        }
                ...
                const startDrag = (e) => {
                    isDragging = true;
                    // Hentikan sementara animasi otomatis
                ...
                container.addEventListener('mouseleave', stopDrag); // Jika mouse keluar area
        ...
        // --- END: KODE JAVASCRIPT CAROUSEL ---
    });

    // Luxury Counter Animation
    function animateLuxuryCounters(entries, observer) {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                let current = 0;
                const increment = target / 200; // Animate over 200 steps
    ...
    if (counters.length > 0) {
        const observer = new IntersectionObserver(animateLuxuryCounters, {
            threshold: 0.5
        });
    ...
    window.addEventListener('scroll', () => {
        ...
        const scrollBar = document.querySelector('.luxury-scroll-bar');
        if(scrollBar) {
    ...
    document.querySelectorAll('.testimonial-luxury').forEach(testimonial => {
        testimonial.addEventListener('mouseenter', () => {
            ...
            if(avatar) {
                ...
                avatar.style.boxShadow = '0 0 20px var(--gold)';
            }
        });
