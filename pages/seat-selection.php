<?php
// seat-selection.php
include '../includes/config.php'; // Pastikan path ini benar relative terhadap seat-selection.php

// Redirect if not logged in or no reservation data from home.php
if(!isset($_SESSION['user_id']) || !isset($_SESSION['reservation_data'])) { //
    header("Location: ../auth/login.php");
    exit();
}

// Process booking form from home.php (store all reservation details)
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    // If coming from home.php (first form submission for reservation details)
    // We already stored it in $_SESSION['reservation_data'] in home.php
    // So, we just ensure $_SESSION['reservation_data'] is set from home.php's POST processing.
    // No need to re-process $_POST directly here.
}
...
function renderTable($tableId, $seats, $isAvailable) {
    ...
    switch($seats) {
        ...
        case 12: // Asumsi Anda mungkin ingin meja 12-kursi juga
            ...
            <div class="restaurant-layout mx-auto mb-12 relative bg-black/95 rounded-lg p-8 border border-gold/30" style="width: 800px; height: 600px;">
                ...
                <div class="grid grid-cols-3 gap-12 h-full pt-16 pb-8 px-8">
                    <div class="flex flex-col justify-between">
                        <?php echo renderTable(1, 2, isset($availableTables[1])); ?>
                        <?php echo renderTable(4, 6, isset($availableTables[4])); ?>
                        <?php echo renderTable(7, 4, isset($availableTables[7])); ?>
                    </div>

                    <div class="flex flex-col justify-between items-center">
                        <?php echo renderTable(2, 2, isset($availableTables[2])); ?>
                        <?php echo renderTable(5, 8, isset($availableTables[5])); ?>
                        <?php echo renderTable(8, 4, isset($availableTables[8])); ?>
                    </div>

                    <div class="flex flex-col justify-between">
                        <?php echo renderTable(3, 4, isset($availableTables[3])); ?>
                        <?php echo renderTable(6, 6, isset($availableTables[6])); ?>
                        <?php echo renderTable(9, 2, isset($availableTables[9])); ?>
                    </div>
                </div>
            </div>
                ...
                <form id="seat-form" action="payment.php" method="POST">
                    ...
                    <?php foreach ($reservation as $key => $value): ?>
                        <?php if (is_array($value)): ?>
                            <?php foreach ($value as $itemIndex => $item): ?>
                                <?php foreach ($item as $itemKey => $itemValue): ?>
                                    <input type="hidden" name="<?php echo $key; ?>[<?php echo $itemIndex; ?>][<?php echo $itemKey; ?>]" value="<?php echo htmlspecialchars($itemValue); ?>">
                                <?php endforeach; ?>
                            <?php endforeach; ?>
                        <?php else: ?>
                    ...
                    <button type="submit" id="proceed-to-payment" class="px-8 py-3 bg-gold text-black hover:bg-transparent hover:text-gold hover:border hover:border-gold transition duration-500">
                        Proceed to Payment
                    </button>
                </form>
            </div>
...
// Add this inside your table click event handler in seat-selection.php
document.getElementById('seat-form').addEventListener('submit', function(e) {
    // Optionally validate selection here
    ...
    // You could also send the data via AJAX to store in session
    fetch('update-session.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: 'table_number=' + selectedTableInput.value
    });
});
});
</script>